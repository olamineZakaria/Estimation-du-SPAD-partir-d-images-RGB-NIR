...
DEFAULT_MODEL  = DEFAULT_MODELS_DIR / "speculaire_final.pt"
DEFAULT_DEVICE = "cuda"
DEFAULT_STRIDE = 32


class SpecularMaskPredictor(BaseMaskPredictor):
    pass


@lru_cache(maxsize=1)
def _get_predictor(model_path: Path, tile_size: int = 512, tile_stride: int = 256) -> SpecularMaskPredictor:
    return SpecularMaskPredictor(model_path=model_path, tile_size=tile_size, tile_stride=tile_stride)
...
def _apply_vegetation_mask(image_path: Path) -> np.ndarray:
    ...
    rgb[veg_mask == 0] = 0  # background → black before specular inference
    return rgb
...
def process_dataset(data_root: Path = DEFAULT_DATA_ROOT, model_path: Path = DEFAULT_MODEL, max_images: int = None, tile_size: int = 512, tile_stride: int = 256) -> None:
    ...
    run_dataset(predictor, data_root, mask_subdir="speculaires", get_input=_apply_vegetation_mask, max_images=max_images)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Specular mask predictor — génère un masque binaire depuis une image RGB",
    ...
    predictor = SpecularMaskPredictor(
