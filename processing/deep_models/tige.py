...
DEFAULT_MODEL  = DEFAULT_MODELS_DIR / "MAnet_pvt_v2_b2_ble_tige_2026_04_22.torchscript"
DEFAULT_DEVICE = "cuda"
DEFAULT_STRIDE = 32


class TigeMaskPredictor(BaseMaskPredictor):
    def __init__(
        self,
...
@lru_cache(maxsize=1)
def _get_predictor(model_path: Path) -> TigeMaskPredictor:
    return TigeMaskPredictor(model_path=model_path)
...
def process_dataset(data_root: Path = DEFAULT_DATA_ROOT, model_path: Path = DEFAULT_MODEL, max_images: int = None) -> None:
    ...
    run_dataset(predictor, data_root, mask_subdir="tige", max_images=max_images)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Tige mask predictor — génère un masque binaire depuis une image RGB",
